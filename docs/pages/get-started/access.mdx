...
Kubernetes cluster owned by the Platform Team in the staging environment. On
staging servers, this role can assume the `manager` user and, on databases, the
privileged `dbuser` role:
