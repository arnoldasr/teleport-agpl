...
// applyTeleportBlock merges the teleport block of a scoped role into the provided classic role
...
func applyTeleportBlock(src *scopedaccessv1.ScopedRoleTeleport, dst *types.RoleConditions) {
...
func ScopedRoleToRole(sr *scopedaccessv1.ScopedRole, assignedScope string) (types.Role, error) {
	...
	applyTeleportBlock(sr.GetSpec().GetTeleport(), &conditions)
