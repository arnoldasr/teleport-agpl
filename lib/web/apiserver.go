...
func (h *Handler) bindDefaultEndpoints() {
	...
	h.GET("/v1/webapi/oidc/login/web", h.WithRedirect(h.oidcLoginWeb))
...
func setEntitlementsWithLegacyLogic(webCfg *webclient.WebConfig, clusterFeatures proto.Features) {
	...
	if ent := clusterFeatures.GetEntitlements(); len(ent) > 0 {
		...
		webCfg.Entitlements[string(entitlements.OIDC)] = webclient.EntitlementInfo{Enabled: true}
		...
		webCfg.Entitlements[string(entitlements.SAML)] = webclient.EntitlementInfo{Enabled: true}
...
func (h *Handler) oidcCallback(w http.ResponseWriter, r *http.Request, p httprouter.Params) string {
	...
	if response.Req.CreateWebSession {
		logger.InfoContext(r.Context(), "OIDC: Redirecting to web browser")
	...
	logger.InfoContext(r.Context(), "OIDC callback redirecting to console login")
	if len(response.Req.SSHPubKey)+len(response.Req.TLSPubKey) == 0 {
		logger.ErrorContext(r.Context(), "Not a web or console login request")
		return client.LoginFailedRedirectURL
	}
